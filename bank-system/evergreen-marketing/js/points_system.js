...
class PointsSystem {
  ...
  async redeemReward(rewardName, pointsCost) {
    ...
    if (this.totalPoints < pointsCost) {
      this.showErrorMessage(
        `Insufficient points! You need ${pointsCost} points but only have ${this.totalPoints.toFixed(
          2
        )} points.`
      );
      return false;
  ...
  async loadMissions() {
    try {
      console.log(
        "Loading missions from:",
        `${this.apiUrl}?action=get_missions`
      );
  ...
  async collectMission(missionId, buttonElement) {
    ...
    try {
      ...
      if (data.success) {
        ...
        const pointsElements = document.querySelectorAll(
          "#totalPoints, .points-number, .points-value"
        );
  ...
  async loadCompletedMissions() {
    try {
      const response = await fetch(
        `${this.apiUrl}?action=get_completed_missions`,
        {
          credentials: "same-origin",
        }
      );
  ...
  showRedemptionSuccess(rewardName, pointsCost) {
    ...
    if (modal && successTxt && txt) {
      ...
      if (!document.querySelector(".points-deducted")) {
        ...
        pointsDeducted.style.cssText =
          "color: #dc3545; font-size: 14px; margin-top: 10px; font-weight: 600;";
        pointsDeducted.textContent = `-${pointsCost} points`;
        txt.parentElement.insertBefore(pointsDeducted, okBtn);
      } else {
        document.querySelector(
          ".points-deducted"
        ).textContent = `-${pointsCost} points`;
      }
    ...
    missions.forEach((mission) => {
      ...
      card.innerHTML = `
                    ...
                    <div class="mission-points">
                        <div class="mission-points-value">${parseFloat(
                    ...
                    <div class="mission-details">
                        ...
                        <div class="mission-actions">
                            <button class="collect-btn" 
                                    onclick="pointsSystem.collectMission(${
                                      mission.id
                                    }, this)"
                                    ${
                                      mission.status === "pending"
  ...
  async renderPointHistory(containerId) {
    ...
    history.forEach((item) => {
      ...
      card.innerHTML = `
                    ...
                    <div class="mission-points">
                        <div class="mission-points-value" style="color: ${pointsColor};">${pointsSign}${Math.abs(
        points
      ).toFixed(2)}</div>
                        <div class="mission-points-label">points</div>
                    </div>
