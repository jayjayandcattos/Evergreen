...
const allInputs = loanForm
  ? loanForm.querySelectorAll("input, select, textarea")
  : [];
...
function updateProgress() {
  ...
  const accountInputs = accountSection
    ? accountSection.querySelectorAll(
        "input[required], select[required], textarea[required]"
      )
    : [];
  const loanInputs = loanSection
    ? loanSection.querySelectorAll(
        "input[required], select[required], textarea[required]"
      )
    : [];
...
function validateAll() {
  ...
  if (fullNameInput) {
    runValidation(() =>
      validateField(
        ...
        3,
        50,
        "Full Name must be 3-50 letters and spaces only.",
        "name-error"
      )
    );
  }
  ...
  if (contactInput) {
    runValidation(() =>
      validateField(
        ...
        null,
        null,
        "Enter a valid Contact Number (e.g., +63 912 345 6789).",
        "contact-error"
      )
    );
  }
  ...
  if (amountInput) {
    ...
    if (isNaN(amount) || amount < 5000) {
      showValidationError(
        amountInput,
        "Loan Amount must be a number and at least ₱5,000.",
        "amount-error"
      );
      isValid = false;
    } else {
  ...
  if (purpose && purpose.value.trim() === "") {
    showValidationError(
      purpose,
      "Please describe the loan purpose.",
      "purpose-error"
    );
    isValid = false;
  } else {
  ...
  if (attachmentInput) {
    if (attachmentInput.files.length === 0) {
      showValidationError(
        attachmentInput,
        "A document attachment is required.",
        "attachment-error"
      );
      isValid = false;
    } else {
